            ...
            <tbody>
              {visible.map((row) => (
                <tr key={row.orderId} className="border-t border-slate-100">
                  ...
                  <td className="px-3 py-3">
                    <button type="button" onClick={() => setSelectedOrder(row)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
            <div className="border-b border-slate-200 px-6 py-4">
              ...
              <p className="text-sm text-slate-500">Complete order information</p>
            </div>
            <div className="space-y-5 p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="rounded-xl bg-slate-50 p-3">
                  ...
                  <p className="font-semibold">{selectedOrder.deliveryDate || "Not Assigned"}</p>
                </div>
                <div className="rounded-xl bg-slate-50 p-3">
              ...
              <div>
                <h4 className="mb-4 font-semibold text-slate-900">Order Status Timeline</h4>
                <div className="flex items-center justify-between">
                  {["Pending", "Approved", "In Transit", "Delivered"].map((step, index) => {
                    const currentIndex = ["Pending", "Approved", "In Transit", "Delivered"].indexOf(selectedOrder.status);
                    const completed = index <= currentIndex;
                    return (
                      <div key={step} className="flex flex-1 flex-col items-center">
                        <div className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-semibold ${completed ? "bg-[#0A3A92] text-white" : "bg-slate-200 text-slate-500"}`}>
                          {index + 1}
                        </div>
                        <p className="mt-2 text-xs text-center text-slate-600">{step}</p>
                      </div>
                    );
                  })}
                </div>
